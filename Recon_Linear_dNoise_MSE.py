...
train_id = '_'.join(['Recon', 'Linear', 'dNoise', 'MSE'])
...
lossFun = MultiLoss(lossFunList=[reconLoss])

preprocess = PrepDist(N, add_noise=True)
...
param = [(N + in_dim, L)  for L in range(5, 6) for N in range(8, 73, 16)]
