...
train_id = '_'.join(['Coord', '1aRNN', 'DC', 'MSE'])
...
def get_model(neuron, i):
    ...
    return rnn.aRNN(
        N=N,
        in_dim = in_dim, 
        output_layer_dim=[mid1, *mid, mid2, out_dim],
        num_rnn_layers=1)
...
param = [(N + in_dim, L) for N in range(40, 73, 16) for L in range(2, 6)]
